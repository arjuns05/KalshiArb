...
import { fetchOddsApiMarkets } from "./connectors/oddsapi.js";
...
export async function poll() {
  ...
  try {
    ...
    const [kalshi, odds, poly] = await Promise.allSettled([
      fetchKalshiMarkets(),
      fetchOddsApiMarkets(),
    ...
    if (odds.status === "rejected") {
      console.error("[Polling] OddsAPI fetch failed:", odds.reason);
    }
    ...
    lastPollResult = {
      ...
      errors: {
        ...
        oddsapi: odds.status === "rejected" ? odds.reason?.message : null,
